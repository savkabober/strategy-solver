...
void mutate_change_time(Trajectory& t) {
    ...
    if (t[idx].time < 0.1) t[idx].time = 0.1;//min time
}
...
void mutate(Trajectory& trajectory) {
    ...
    } else if (choice < 0.8) { // 40%
        mutate_change_time(trajectory);
    } else if (choice < 0.95) { // 15%
...
int main()
{
    ...
    Risovalka drawer = Risovalka(950,650);
    ...
    while (drawer.window.isOpen())
    {
        ...
        for(double t = 0;t<total_time;t+=0.01)
        {
            ...
            Point acc = rotate(Point(MAX_ACCELERATION,0),cur_wp.acc_angle);
